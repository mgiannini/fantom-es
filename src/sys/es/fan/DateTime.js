...
//////////////////////////////////////////////////////////////////////////
  ...
  /*
  // TODO:FIXIT - implement this class
  static make(year, month, day, hour, min, sec=0, ns=0, tz=TimeZone.cur()) {
    return DateTime.doMake$(year, month, day, hour, min, sec, ns, undefined, tz);
  }

  static doMake$(year, month, day, hour, min, sec, ns, knownOffset, tz) {
    month = month.ordinal();
    ...
    const dayOfYear = DateTime.dayOfYear(year, month, day);
    const timeInSec = hour*3600 + min*60 + sec;
    let ticks = Int.plus(DateTime.yearTicks[year-1900],
                Int.plus(dayOfYear * Duration.nsPerDay$,
                Int.plus(timeInSec * Duration.nsPerSec$, ns)));

    // adjust for timezone and dst (we might know the UTC offset)
    // TODO:FIXIT FROM HERE
    const rule = tz.rule(year);
    let dst;
    if (knownOffset == null) {
